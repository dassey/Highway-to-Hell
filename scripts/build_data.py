...
def read_rows(zf, base):
    ...
    with zf.open(name) as raw:
        ...
        for row in csv.DictReader(text):
            yield {norm_key(k): (v or "").strip() for k, v in row.items()}
...
def build_year(year, zf, harvest, points, shards_dir, states, state_idx, stfips,
    ...
    for row in read_rows(zf, "accident"):
        ...
        if hour == 24:  # some early years code midnight as 24
            hour = 0
        if not (0 <= hour <= 23):
            hour = -1
...
def main():
    ...
    ap.add_argument("--years", nargs="+", type=int, default=list(range(2001, 2025)))
